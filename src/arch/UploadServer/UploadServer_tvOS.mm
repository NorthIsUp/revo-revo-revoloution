...
static NSString *s_documentsPath = nil;  // app Documents dir; same path mounted at /Songs, /Themes, etc.
...
static NSString *UploadHTML(void)
{
	...
	for (NSString *t in s_allowedTargets) {
		...
		[tabs appendFormat:@"<div class=\"tab\" data-target=\"%@\">"
			@"<h2>%@</h2>"
			@"<div class=\"dropzone\" data-target=\"%@\" ondragover=\"event.preventDefault();event.stopPropagation();this.classList.add('drag-over');this.querySelector('.dropzone-text').textContent='Release to upload';\" ondragleave=\"event.preventDefault();this.classList.remove('drag-over');this.querySelector('.dropzone-text').textContent='Drop folder or files here';\" ondrop=\"handleDrop(event,'%@')\"><span class=\"dropzone-text\">Drop folder or files here</span><span class=\"dropzone-status\" aria-live=\"polite\"></span></div>"
			@"<p><button type=\"button\" onclick=\"document.getElementById('target').value='%@';document.getElementById('files').click()\">Add folder…</button></p>"
			@"</div>",
			t, t, t, t, t];
	}
	return [NSString stringWithFormat:
		...
		@"<style>"
		@"body{font-family:system-ui;margin:1rem;} .tabs{display:flex;flex-wrap:wrap;gap:0.5rem;} .tab{border:1px solid #ccc;padding:1rem;min-width:10rem;} .tab.active{background:#e8f4fc;} "
		@".dropzone{border:2px dashed #999;padding:2rem;margin:0.5rem 0;min-height:5rem;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;transition:border-color .15s,background .15s;} "
		@".dropzone.drag-over{border-color:#06c;border-width:3px;background:#e0f0ff;} "
		@".dropzone.uploading{border-color:#690;background:#f0ffe0;} .dropzone.uploading .dropzone-text{font-weight:bold;} "
		@".dropzone.done{border-color:#060;background:#e8f8e8;} .dropzone.error{border-color:#c00;background:#ffe8e8;} "
		@".dropzone-status{margin-top:0.5rem;font-size:0.9em;} label{display:block;margin:0.5rem 0;} .overwrite-wrap{margin:1rem 0;} .overwrite-wrap .radio{margin:0.25rem 0;}"
		@"</style></head><body><h1>ITGmania Upload</h1>"
		@"<form action=\"/upload\" method=\"post\" enctype=\"multipart/form-data\" id=\"uploadForm\">"
		@"<input type=\"hidden\" name=\"target\" id=\"target\" value=\"Songs\">"
		@"<input type=\"file\" name=\"files\" id=\"files\" multiple webkitdirectory directory style=\"display:none\">"
		@"<div class=\"tabs\">%@</div>"
		@"<p class=\"overwrite-wrap\">When an uploaded folder has the same name:<br>"
		@"<label class=\"radio\"><input type=\"radio\" name=\"overwrite\" value=\"1\"> Replace the current contents with the new folder</label>"
		@"<label class=\"radio\"><input type=\"radio\" name=\"overwrite\" value=\"\" checked> Merge two folders contents</label></p>"
		@"<p><button type=\"submit\">Upload</button></p>"
		@"</form>"
		@"<script>"
		@"function handleDrop(e,target){e.preventDefault();e.stopPropagation();var zone=e.currentTarget;zone.classList.remove('drag-over');zone.querySelector('.dropzone-text').textContent='Drop folder or files here';"
		@"var files=e.dataTransfer.files;if(files.length===0)return;"
		@"zone.classList.add('uploading');zone.classList.remove('done','error');zone.querySelector('.dropzone-status').textContent='Uploading '+files.length+' item(s)...';"
		@"var fd=new FormData();fd.append('target',target);fd.append('overwrite',(document.querySelector('[name=overwrite]:checked')&&document.querySelector('[name=overwrite]:checked').value)||'');for(var i=0;i<files.length;i++)fd.append('files',files[i]);"
		@"fetch('/upload',{method:'POST',body:fd}).then(r=>r.text()).then(function(t){zone.classList.remove('uploading');var status=zone.querySelector('.dropzone-status');"
		@"if(t.indexOf('Saved')>=0){zone.classList.add('done');var n=(t.match(/Saved (\\d+)/)||[])[1]||'?';status.textContent='Saved '+n+' file(s).';}"
		@"else{zone.classList.add('error');status.textContent=(t.indexOf('error')>=0||t.indexOf('Error')>=0)?(t.replace(/<[^>]*>/g,'').trim().slice(0,150)):'Upload failed.';}"
		@"}).catch(function(err){zone.classList.remove('uploading');zone.classList.add('error');zone.querySelector('.dropzone-status').textContent='Error: '+err.message;});}"
		@"</script></body></html>",
		tabs];
}
...
static void StartServerOnMainQueue(void)
{
	...
	// Register static/fallback GET first so it is tried last (handlers are LIFO: last registered = first tried).
	// We want /list, /upload, /delete to take precedence over the base path.
	NSFileManager *fm = [NSFileManager defaultManager];
	NSString *bundleResources = [[NSBundle mainBundle] resourcePath];
	NSString *uploadUIPath = bundleResources != nil
	    ? [bundleResources stringByAppendingPathComponent:@"dist"]
	    : nil;
	...
	[s_server addHandlerForMethod:@"POST"
	                         path:@"/upload"
	                 ...
	                 processBlock:^GCDWebServerResponse *(GCDWebServerRequest *req) {
		                 ...
		                 if (![fm createDirectoryAtPath:targetDir withIntermediateDirectories:YES attributes:nil error:&mkdirErr]) {
			                 LOG->Warn("Upload server: could not create directory '%s': %s",
			                           targetDir.UTF8String, mkdirErr.localizedDescription.UTF8String);
			                 return [GCDWebServerErrorResponse responseWithClientError:kGCDWebServerHTTPStatusCode_BadRequest
			                                                                 message:@"Invalid target"];
		                 }
		                 BOOL overwrite = NO;
		                 GCDWebServerMultiPartArgument *overwriteArg = [request firstArgumentForControlName:@"overwrite"];
		                 if (overwriteArg != nil && overwriteArg.string.length > 0 && [overwriteArg.string.lowercaseString hasPrefix:@"1"])
		                         overwrite = YES;
		                 ...
		                 for (GCDWebServerMultiPartFile *file in request.files) {
			                 ...
			                 if ([fm copyItemAtPath:file.temporaryPath toPath:destPath error:&err]) {
				                 ...
				                 LOG->Info("Upload server: saved '%s' to %s", safeName.UTF8String, target.UTF8String);
			                 } else {
				                 LOG->Warn("Upload server: failed to save '%s': %s",
				                           destPath.UTF8String, err.localizedDescription.UTF8String);
			                 }
		                 }
		                 NSString *body = saved.count > 0
		                     ? [NSString stringWithFormat:@"<p>Saved %lu file(s) to %@.</p>", (unsigned long)saved.count, target]
		                     : @"<p>No files saved.</p>";
		                 return [GCDWebServerDataResponse responseWithHTML:
		                     [@"<!DOCTYPE html><html><body><h1>Upload</h1>" stringByAppendingString:
		                         [body stringByAppendingString:@"<p><a href=\"/\">Back</a></p></body></html>"]]];
	                 }];
	...
	[s_server addHandlerForMethod:@"POST"
	                 ...
	                 processBlock:^GCDWebServerResponse *(GCDWebServerRequest *req) {
		                 ...
		                 for (NSString *seg in segments) {
			                 NSString *s = SanitizePathComponent([seg stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]);
			                 if (s == nil)
				                 return [GCDWebServerErrorResponse responseWithClientError:kGCDWebServerHTTPStatusCode_BadRequest
		                 ...
		                 if (![resolved hasPrefix:resolvedTarget] || [resolved isEqualToString:resolvedTarget]) {
			                 return [GCDWebServerErrorResponse responseWithClientError:kGCDWebServerHTTPStatusCode_BadRequest
			                                                                 message:@"Invalid path"];
		                 }
		                 NSFileManager *fm = [NSFileManager defaultManager];
		                 if (![fm fileExistsAtPath:fullPath]) {
			                 return [GCDWebServerErrorResponse responseWithClientError:kGCDWebServerHTTPStatusCode_NotFound
			                                                                 message:@"Not found"];
		                 }
		                 NSError *err = nil;
		                 if (![fm removeItemAtPath:fullPath error:&err]) {
			                 LOG->Warn("Upload server: failed to delete '%s': %s",
			                           fullPath.UTF8String, err.localizedDescription.UTF8String);
			                 return [GCDWebServerErrorResponse responseWithServerError:kGCDWebServerHTTPStatusCode_InternalServerError
			                                                          underlyingError:err
			                                                                 message:@"Delete failed"];
			                 }
		                 LOG->Info("Upload server: deleted '%s' from %s", relativePath.UTF8String, target.UTF8String);
...
void UploadServer_Start(const std::string &documentsPath)
{
	...
	// GCDWebServer requires first use (including +initialize) on main thread; do all creation on main queue

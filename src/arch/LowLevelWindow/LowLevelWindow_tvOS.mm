...
static bool SetupCVRenderTarget( int w, int h )
{
	CVReturn err = CVOpenGLESTextureCacheCreate( kCFAllocatorDefault, NULL,
		g_EAGLContext, NULL, &g_TextureCache );
	...
	err = CVPixelBufferCreate( kCFAllocatorDefault, w, h,
	...
	err = CVOpenGLESTextureCacheCreateTextureFromImage(
		kCFAllocatorDefault, g_TextureCache, g_PixelBuffer, NULL,
		GL_TEXTURE_2D, GL_RGBA, w, h, GL_BGRA, GL_UNSIGNED_BYTE,
		0, &g_CVTexture );
	if( err != kCVReturnSuccess )
	{
	...
	{
		CFRelease( g_CVTexture ); g_CVTexture = NULL;
		CFRelease( g_PixelBuffer ); g_PixelBuffer = NULL;
		CFRelease( g_TextureCache ); g_TextureCache = NULL;
		return false;
	}
...
static void FreeDataProviderCallback( void *info, const void *data, size_t size )
{
	free( info );
}
...
LowLevelWindow_tvOS::~LowLevelWindow_tvOS()
{
	...
	if( g_DepthRenderbuffer ) { glDeleteRenderbuffers( 1, &g_DepthRenderbuffer ); g_DepthRenderbuffer = 0; }
	if( g_bUsePBO ) { glDeleteBuffers( 2, g_PBO ); g_PBO[0] = g_PBO[1] = 0; }
	if( g_CVTexture ) { CFRelease( g_CVTexture ); g_CVTexture = NULL; }
	if( g_PixelBuffer ) { CFRelease( g_PixelBuffer ); g_PixelBuffer = NULL; }
	if( g_TextureCache ) { CFRelease( g_TextureCache ); g_TextureCache = NULL; }
	free( g_ReadbackBuf ); g_ReadbackBuf = NULL;
	[EAGLContext setCurrentContext:nil];
	g_EAGLContext = nil;
	g_HostView = nil;
}

void *LowLevelWindow_tvOS::GetProcAddress( RString s )
{
	return nil;
}

RString LowLevelWindow_tvOS::TryVideoMode( const VideoModeParams &p, bool &newDeviceOut )
{
	newDeviceOut = false;
	...
	if( newDeviceOut )
	{
		...
		glRenderbufferStorage( GL_RENDERBUFFER, GL_DEPTH_COMPONENT24_OES, g_BackingWidth, g_BackingHeight );
...
void LowLevelWindow_tvOS::SwapBuffers()
{
	...
	if( g_bUsePBO )
	{
		...
		CGDataProviderRef provider = CGDataProviderCreateWithData(
			pixels, pixels, dataSize, FreeDataProviderCallback );
		CGImageRef img = CGImageCreate( w, h, 8, 32, rowBytes, cs,
...
RenderTarget *LowLevelWindow_tvOS::CreateRenderTarget()
{
	return nullptr;
}

...
static void MapButton(
    InputHandler_tvOS* handler, DeviceInput di, float value, float threshold = 0.5f) {
  di.level = value;
...
static void RegisterHandlers(GCController* controller) {
  if (controller.extendedGamepad) {
    ...
    if (@available(tvOS 13.0, *)) {
      ...
      if (gp.buttonOptions) {
        gp.buttonOptions.pressedChangedHandler =
            ^(GCControllerButtonInput* b, float v, BOOL pressed) {
    ...
    LOG->Info(
        "Registered value-changed handlers for extended gamepad '%s'",
    ...
    LOG->Info(
        "Registered value-changed handlers for micro gamepad '%s'",
        controller.vendorName ? [controller.vendorName UTF8String] : "(unknown)");
  }
}
...
InputHandler_tvOS::InputHandler_tvOS() {
  ...
  LOG->Info(
      "InputHandler_tvOS: Initialized. Found %d controller(s).",
      (int)[[GCController controllers] count]);

  for (GCController* c in [GCController controllers]) {
    if (c.extendedGamepad) {
      LOG->Info(
          "  - Extended gamepad: '%s'", c.vendorName ? [c.vendorName UTF8String] : "(unknown)");
    } else if (c.microGamepad) {
      LOG->Info(
          "  - Micro gamepad (Siri Remote): '%s'",
          c.vendorName ? [c.vendorName UTF8String] : "(unknown)");
    } else {
      LOG->Info(
...
void InputHandler_tvOS::GetDevicesAndDescriptions(std::vector<InputDeviceInfo>& vDevicesOut) {
  ...
  for (NSUInteger i = 0; i < [controllers count] && i < (NSUInteger)NUM_JOYSTICKS; i++) {
    ...
    if (c.extendedGamepad) {
      std::string desc =
          ssprintf("MFi Gamepad: %s", c.vendorName ? [c.vendorName UTF8String] : "Unknown");
